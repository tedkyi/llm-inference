...
-->
```
![Static batching](images/under_construction.jpg){#fig-static-batching .lightbox}

The problems are obvious.
Request 1 finishes early but its batch slot sits empty -- the GPU does no useful work for that slot while waiting for Request 3 to complete.
Request 4 is stuck in the queue even though there's a free slot.
If requests have different input lengths, the shorter inputs need to be padded to match the longest one, wasting compute on dummy tokens during prefill.
...
For anything with variable-length outputs, which is the common case in LLM generation, it wastes too many GPU cycles.
...
If only 5 requests are waiting, the batch has 5 requests rather than waiting for a full batch of 32.
...
Short-output requests sit idle while they wait for long-output requests to wrap up.
The GPU is still doing useless work for completed slots.
...
-->
```
![Continuous batching](images/under_construction.jpg){#fig-continuous-batching .lightbox}

This eliminates **head-of-line blocking** -- the problem where a single slow request holds up the entire batch.
In the static batching example, Request 4 had to wait for the entire first batch to complete.
With continuous batching, it gets scheduled into the first available slot as soon as any request finishes.
...
-->
```
![Ragged batching](images/under_construction.jpg){#fig-ragged-batching .lightbox}
...
-->
```
![Chunked prefill](images/under_construction.jpg){#fig-chunked-prefill .lightbox}
...
In practice, chunk sizes of 256--2,048 tokens are common, depending on the model size and the latency sensitivity of the workload.
...
The prefill chunk tokens keep the compute units busy while the decode tokens make progress.
...
-->
```
![Disaggregated prefill](images/under_construction.jpg){#fig-disaggregated-prefill .lightbox}
