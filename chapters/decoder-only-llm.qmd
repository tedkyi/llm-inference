...
The "N×" is shorthand, and it saves space in diagrams, but in the real world, such as on GPU memory, all of those many copies do exist and take up a lot of room.
...
We are modeling this conceptually at this time, but we will see in @sec-kv-cache-intro that a fundamental optimization will implement this orange path in data store called the KV cache.
...
This idea of multiple copies of our model is a good mental model to understand data flow across token positions, however this is not how model inference happens.
...
Passing in this 2D tensor into a single copy of the model yields a single tensor with outputs identical to what multiple copies of the model would have produced.
...
When embedding tokens, the **([V]{.dim-v}**, **[D]{.dim-d})** weights are used, and when obtaining logits, the transpose of these weights are used.
...
There isn't much to optimize being shown here.
...
Its size is quadratic in sequence length. For a 4096-token prompt, each head produces a 4096×4096 matrix, and when **[S]{.dim-s}** = 100,000, each head's attention matrix is 100,000x100,000 and has 10 billion entries.
...
![The KV cache avoids wasted recomputation of keys and values which never change over and over again](images/kv_cache_concept.svg){#fig-kv-cache .lightbox}
...
This eliminates the problem of KV caches growing large, giving a fundamentally different inference profile --- constant memory per step instead of linearly growing memory.
...
It's worth knowing that alternative exists, especially since the KV cache is a major source of inference complexity, but so far nothing less expensive has matched the capabilities of multi-head attention.
