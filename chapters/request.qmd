...
The result: HBM traffic drops from $O(S^2)$ to $O(\dims{\text{S}} \cdot \dimdk{d_K})$ for the attention matrix, where $\dimdk{d_K}$ is the head dimension (typically 64 or 128).
