...
HBM traffic drops from $O(S^2)$ to $O(\dims{\text{S}} \cdot \dimdk{d_K})$ for the attention matrix, where $\dimdk{d_K}$ is the head dimension (typically 64 or 128).
...
During prefill, the sequence length **[S]{.dim-s}** is large (the full prompt), so the $O(\dims{\text{S}}^2)$ savings are dramatic.
During decode, **[S]{.dim-s}** is just 1 for the query (the new token), but the key and value sequences span the entire context, so the tiled approach still avoids unnecessary intermediate writes.
...
For a model with **[L]{.dim-l}** layers, $n_{kv}$ KV heads per layer, and head dimension $\dimdk{d_K}$, each token adds the following to the KV cache:

$$\text{bytes per token} = 2 \times \diml{\text{L}} \times n_{kv} \times \dimdk{d_K} \times b_{\text{dtype}}$$

The factor of 2 accounts for both keys and values. $b_{\text{dtype}}$ is the bytes per element (2 for FP16/BF16, 1 for FP8/INT8).

For a concrete example, consider a 70B parameter model with [L]{.dim-l} = 80 layers, $n_{kv} = 8$ GQA heads (see @sec-efficient-attention), $\dimdk{d_K} = 128$, stored in FP16:
...
This wastes memory in three ways, illustrated in @fig-kv-waste: **reserved** slots held for tokens that haven't been generated yet, **internal fragmentation** (allocated slots that will never be used because the sequence won't reach the maximum length), and **external fragmentation** (gaps between allocations that are too small to be useful).
...
Rather than simply choosing a new datatype, they use techniques like rotations and vector quantization to compress entries.
They report success squeezing entries down to 3.5 bits without hurting LLM performance.
Given that decoding is bandwidth bound, reductions in KV cache size provide close to linear speedups.
...
![Speculative decoding](images/speculative_decoding.svg){#fig-speculative-decoding .lightbox}
