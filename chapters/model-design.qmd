...
Compared to weight-only quantization, the greatest benefit of weight-activation quantization is that speeding up the matrix multiplication shifts the ridge point in the roofline model to the right and improves MFU for compute-bound workloads.
...
The motivation is similar to quantization: fewer parameters means less data to move, which speeds up bandwidth-bound decode, resulting in a lower TPOT.
...
For most practical scenarios, the efficiency gains from fewer decode steps outweigh the increased memory footprint and minute TPOT increase from the larger embedding and LM head.
This is one reason why the trend in recent LLMs has been toward larger vocabularies.^[Larger vocabularies also improve multilingal performance for languages such as Chinese, which do not share an alphabet with English.]
