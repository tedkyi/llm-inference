...
On pruning, two papers go well beyond the overview in this chapter.
SparseGPT [@frantar2023sparsegpt] showed that models up to 176B parameters can be pruned to 50--60% sparsity in a single pass with minimal quality loss, and that the pruning pattern can be constrained to NVIDIA's 2:4 structured format for hardware acceleration.
...
For a deeper understanding of Multi-head Latent Attention beyond its brief treatment here, Eryk Banatt's "Understanding Multi-Head Latent Attention" [@banatt2025mla] is a clear technical walkthrough of the low-rank KV compression, the decoupled rotary position embedding trick, and how MLA compares to GQA in practice.
...
For knowledge distillation applied to LLMs, @xu2024distillsurvey is the most comprehensive recent survey, covering the full taxonomy from white-box distillation (where student models train on the teacher's logits) to black-box distillation (where they learn from the teacher's generated outputs), along with domain-specific and skill-specific distillation techniques.

For broader surveys that cover multiple topics from this chapter:

- @wan2024efficient is the most comprehensive survey of LLM efficiency techniques, covering quantization, pruning, distillation, and efficient architectures from model-centric, data-centric, and framework-centric perspectives.
