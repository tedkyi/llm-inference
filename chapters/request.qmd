...
-->
```
![Flash attention](images/under_construction.jpg){#fig-flash-attention .lightbox}

The problem is the intermediate matrices $S$ and $P$.
They each have $S^2$ elements per head, and they are materialized in HBM just to be read back in the next step.
For a sequence of length 4,096, the score matrix alone is $4096 \times 4096 = 16.7$ million entries per head.
The actual computation -- the matrix multiplications and softmax -- is fast.
...
The GPU spends most of its time waiting for data to move between SRAM and HBM, not doing arithmetic.
...
The core idea is **tiling**: break the $Q$, $K$, and $V$ matrices into small blocks that fit in SRAM, then compute the attention output block by block, keeping all intermediate results on-chip.
