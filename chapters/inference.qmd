...
Since the attention scores are a square whose sides are the number of tokens, this grows quadratically with respect to the sequence length, and it becomes very expensive when there are many input tokens.
...
After prefill completes, the system has two things: the KV cache populated with entries for all input tokens and the logits for the last input position's next token prediction, which are used to generate the first output token.

::: callout-note
For a detailed look at exactly how the key, value, and query projections are computed during the transformer's forward pass and how they relate to the KV cache, see sections [-@sec-self-attention] and [-@sec-kv-cache-intro].
:::
...
Sampling is not where the time goes.
...
This is usually a hard constraint that is set up front in the serving framework at the time the model is loaded.
