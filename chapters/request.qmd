...
This is why this implementation of attention causes GPU out of memory errors for long sequences.
...
FlashAttention's tiling is most effective during prefill, where the sequence length **[S]{.dim-s}** is large (the full prompt) and the $O(\dims{\text{S}}^2)$ memory traffic savings are dramatic.
...
At 5 $\mu$s each, that's hundreds of microseconds per token of pure overhead --- time the GPU spends idle waiting for the CPU to tell it what to do next.
...
- **Near-zero fragmentation**: no external fragmentation, and the only internal fragmentation is the partial last page of each request. With 16-token pages, average waste is 8 tokens per request --- negligible compared to the old approach of reserving memory for the maximum sequence length.
...
The combination of paged attention and ragged batching (discussed in @sec-ragged-batching) is a beautiful solution that reduces both KV cache memory usage and memory reads.
...
Interest is high because decoding being bandwidth bound means that reductions in KV cache size provide close to linear speedups to TPOT and throughput.
...
Adding draft model on top of a high MFU pipeline provides less benefit and can actually hurt throughput.
