...
We will also mention here that the input to the first layer is a vector of floating point numbers of length $d_{model}$, which will will abbreviate **([D]{.dim-d})**.
...
This first tensor on the bottom has shape **([S]{.dim-s}, [D]{.dim-d})**, where **[S]{.dim-s}** is the the number of rows and **[D]{.dim-d}** is the number of columns.
...
The technical details and pros and cons of different position embedding techniques are beyond scope of this section. Some approaches worth mentioning are:

**Sinusoidal position embeddings**: the position embeddings used in the original "Attention is All You Need" paper. 
A special embedding table of shape **(max_seq_len, [D]{.dim-d})** is added to the token embeddings before they are input the the first decoder layer. 
...
These are called sinusoidal because the values in each length [D]{.dim-d} position vector are based on the trigonemetric functioins sine and cosine.
...
In addition, dashed lines on the weight matrics and Q, K, and V tensors show the conceptual division of those tensors into heads, even before the reshape.
...
The matrix multiplication $Q K^T$ produces a tensor of shape **([H]{.dim-h}, [S]{.dim-s}, [S]{.dim-s})**. For each head, we get a matrix shape **([S]{.dim-s}, [S]{.dim-s})** where entry $(i, j)$ is the raw attention score between the query at token position $i$ and the key at token position position $j$.
...
The full self-attention block transformed and input of shape **([S]{.dim-s}, [D]{.dim-d})** to an output of the same shape **([S]{.dim-s}, [D]{.dim-d})**.
...
-->
```
![KV cache. TODO: this diagram needs to be completely redesigned.](images/kv_cache.svg){#fig-kv-cache .lightbox}
...
The FFN layers have double the number of parameters as the attention layers, so the FFNs comprise about two-thirds of the parameters, and the attention layers have about one-third of the paramters in a vanilla, traditional LLM.
...
- Jay Alammar's "The Illustrated Transformer" [@alammar2018illustrated] is considered a classic for explaining the original transformer architecture, with many beatiful diagrams.
