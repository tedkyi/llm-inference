...
This stage is fast relative to the resource-intensive work involving the neural network model that follows and is typically handled on the CPU.
...
They keys are still a matrix of $S$ vectors, but now they're being multiplied by a single query vector instead of a matrix of $S$ vectors.
...
The model has finished because it either generated a stop token, hit a maximum length limit, or been interrupted by the serving system.
...
Since the system has to process the entire input before it can begin generating any output, longer prompts means longer prefill, which means higher TTFT.

Note that TTFT isn't as simple as the prefill duration plus fixed costs for the other phases.
In systems under heavy load, the request may sit in queue before being serviced.
...
In production serving, batch configuration significantly impacts scheduler performance.
...
You get roughly double the throughput when you half the precision.
...
As the closest levels are the fastest but also the smallest, and as the levels get larger, they get farhter and slower, trading off speed for capacity.
...
The GPU is told what to dy by the CPU.
...
During prefill, the model processes $S$ input tokens in one forward pass.
...
where $P$ is the number of model parameters and $S$ is the number of input tokens.
...
For very long sequences, the $S^2$ in the attention cost becomes significant, and adds even more to the arithmetic intensiry.
:::
...
Very short prompts could push prefill toward the memory-bandwidth-bound side, though in practice long system prompts tend to prevent requests from every being this short.
...
The KV cache analysis in this chapter is a simplified version of what we'll cover in much more detail in @sec-kv-cache.
For now, the key takeaway is the memory formula: $2 \times L \times H_{\text{kv}} \times d_{\text{head}} \times b_{\text{kv}} \times S$ bytes per request.
Readers who want to understand the full implications of this formula for system design should look ahead to the PagedAttention paper [@kwon2023vllm], which addresses the memory fragmentation problem that arises when managing KV caches across many concurrent requests.
