...
The optimizations in this chapter are all about the model itself, separate from request handling.
...
Some of these are choices made by model designers prior to training.
Others are applied post-training to an existing model.
...
But understanding these techniques is still important, whether you have any say in the model or not.
...
During decode, the bottleneck is memory bandwidth.
...
On an H100 with 3.35 TB/s of bandwidth, reading 140 GB takes about 42 milliseconds -- and that's a hard floor on your per-token latency, ignoring everything else.
...
That's a 4x improvement in your theoretical best-case decode speed, and you didn't change a single line of serving code.

The key insight is that quantization doesn't just reduce the amount of GPU memory needed to store the model weights (which is a pretty big benefit).
...
If we have both and offset and a scaling parameter, then when $x$ is the higher precision value and $\tilde{x}$ is the quantized value, the conversion is $x = (\tilde{x} - \text{offset}) * \text{scaling}$.
This dequantization step also adds a small amount of compute overhead to model inference.

LLMs tolerate quantization well.
...
Weight-only quantization reduces memory bandwidth demand but doesn't speed up the actual matrix multiplication -- the compute still runs at FP16.
...
This means W8A8 doesn't just reduce memory traffic -- it also increases the compute ceiling.
As with weight-only quantization, there is some addition storage and compute overhead for the conversion parameters.
...
In this scenario, one very large value distorts the conversion scale.
...
Simple PTQ simply analyzes the weights, and more advanced techniques require processing a small set of calibration data through the model so activation information can also be analyzed.
...
While effective, the extra work required has limited adoption.
...
Here are a few post-training quantization methods and how they try to avoid activation outliers.
...
GPTQ can be applied to a pre-trained model without any additional training data -- just a small calibration set.
...
In @sec-metrics, we introduced Model Bandwidth Utilization (MBU) as a key diagnostic for decode performance.
Quantization improves the *theoretical* decode speed by reducing the bytes that need to be read per token.
But it doesn't automatically improve MBU --- if your serving system has overhead that prevents it from saturating HBM bandwidth, quantization reduces the numerator (bytes of model data) but the denominator (peak bandwidth) stays the same, so the time improvement is proportional to the data reduction only if MBU remains constant.

In practice, quantized models often achieve *higher* MBU because the smaller data footprint means fewer cache misses, better memory access patterns, and the possibility of fitting the entire model in a single GPU's memory rather than splitting across devices.
...
In the original distillation formulation [@hinton2015distilling], the student learns not just from the ground-truth labels, but from the teacher's full output distribution -- the probability the teacher assigns to every token in the vocabulary, not just the top-1 prediction.
...
Newer techniques vary the approach by training on sample outputs from the larger model or using reinforcement learning to impart some of the larger models capability into the smaller model.

From an inference perspective, the result is simple: you get a smaller model.
Fewer parameters means less data to read from HBM per decode step, a smaller KV cache (if the model also has fewer layers or a smaller hidden dimension), and less memory consumed overall.
...
This may not be an option for a frontier model, but distillation is popular in research and in use for smaller models.
...
Standard GPU hardware and matrix multiplication kernels still process the full dense matrices, so they can't exploit this sparsity efficiently, and sparse matrix multiplications are much slower than their dense counterparts, so they need extremely sparse matrices before they are faster than full dense matrix multiplication.
To get real speedups from unstructured pruning, you need specialized sparse compute support, such as NVIDIA's 2:4 structured sparsity format on Ampere and later GPUs, and it is more difficult to prune weights into this pattern.

### Structured pruning

**Structured pruning** takes a coarser approach: instead of zeroing individual weights, it removes entire structural units -- attention heads, entire layers, or neurons in MLP blocks.
...
This section covers architecture choices -- decisions made during model design and training that affect inference efficiency from the start.

MLP layers account for roughly two-thirds of a typical transformer's parameters, but they operate independently on each token.
Their cost scales linearly with sequence length, and they are straightforward to parallelize.
...
Reducing the KV cache per request lets you serve more concurrent requests in the same memory budget.
...
-->
```
![Techniques for reducing the KV cache size of a given layer](images/under_construction.jpg){#fig-attn-head-variants .lightbox}
...
For a model with 32 heads and a head dimension of 128, each token adds $2 \times 32 \times 128 = 8{,}192$ values to the KV cache per layer.
This is the most expensive option.

**Multi-Query Attention (MQA)** [@shazeer2019mqa] goes to the opposite extreme: all attention heads share a *single* set of key and value projections.
...
This reduces the KV cache by a factor equal to the number of heads -- from 32 sets of K/V down to 1 in the example above.
The quality impact is generally small for large models, and the inference speedup -- especially for long sequences with many concurrent requests -- can be substantial.
...
It offers most of MQA's memory savings with less quality risk.
...
The tradeoff is additional compute to decompress the latent vectors during attention, but this compute is typically small relative to the bandwidth savings.

![Cross-layer attention](images/under_construction.jpg){#fig-cross-layer-attn .lightbox}
...
![Sliding window attention](images/under_construction.jpg){#fig-sliding-window-attn .lightbox}
...
This reduces the attention computation from $O(n^2)$ to $O(n \times w)$, where $w$ is the window size.
...
![Parallel attention and MLP blocks](images/parallel_block.svg){#fig-attn-head-variants .lightbox}

**Parallel attention and MLP blocks** is a simpler architectural trick used in some models, including PaLM [@chowdhery2022palm].
...
These are an active area of research, but the vast majority of deployed LLMs today use softmax attention, so we'll focus our attention there.
:::
...
A larger vocabulary means each token covers more text on average -- common words or subwords that would otherwise be split into multiple tokens get their own single token.
...
At FP16, that's about 1 GB -- not negligible, but typically a small fraction of the total model size for large LLMs.
...
This is why the trend in recent LLMs has been toward larger vocabularies.

## Summary

The techniques in this chapter all reduce what the hardware has to do for each token.
Quantization reduces the bytes-per-parameter, directly attacking the bandwidth bottleneck that dominates decode.
