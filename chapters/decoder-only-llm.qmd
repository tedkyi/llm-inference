...
Note that there is a router in every layer, so the choice of experts differs each layer --- this is not at all like having **E** separate full models which are chosen at the beginning on a per token basis.
...
They also have the concepts of always-on experts called **shared experts**. @Fig-moe-flow shows what the vanilla data flow looks like with an MoE configuration with 6 total experts, 2 of which are active.
