...
![The stages of LLM inference for a single request. Model loading is a one-time setup cost. Ingestion and response completion are fast. Prefill and decode dominate wall-clock time.](images/pipeline_stages.svg){#fig-pipeline-stages}
...
![Request timeline showing TTFT and TPOT durations](images/ttft_tpot_timeline.svg){#fig-ttft-tpot}
...
![The four workload quadrants defined by input and output length](images/io_quadrants.svg){#fig-io-quadrants}
...
![GPU memory hierarchy showing registers, SRAM, L2 cache, and HBM with capacity and bandwidth at each level](images/gpu_memory_hierarchy.svg){#fig-gpu-memory-hierarchy}
...
![Roofline plot for the H100 SXM at FP16](images/roofline.svg){#fig-roofline}
...
![Roofline plot showing decode operating points at different batch sizes. Each point sits on the roofline at arithmetic intensity equal to the batch size (for FP16 weights). Batching moves the operating point rightward, from deeply bandwidth-bound toward the compute ceiling.](images/roofline_batching.svg){#fig-roofline-batching}
