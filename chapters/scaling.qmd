...
```{=html}
<!-- Figure: 8-GPU DGX H100 node topology
ASCII art sketch:
        ...
        └──┬───┬───┬───┬───┬───┬───┬───┬───────┘
         ...
         └─┬─┴─┬─┴─┬─┴─┬─┴───┴───┴───┴───┘
           │   │   │   │
         ┌─┴───┴───┴───┴─┐
         │   PCIe Switch   │──── CPU, NIC, NVMe
         └─────────────────┘

  Each GPU ↔ NVSwitch: 900 GB/s bidirectional (NVLink)
  Each GPU ↔ PCIe:     128 GB/s bidirectional (Gen5 x16)
-->
```
![Intra-node and inter-node connections](images/under_construction.jpg){#fig-interconnect .lightbox}
...
With $H$ attention heads split across $T$ GPUs, each GPU handles $H/T$ heads.
...
$$V_{\text{all-reduce}} = 2 \times \frac{T-1}{T} \times d_{\text{model}} \times b \times \text{bytes\_per\_element}$$

where $b$ is the batch size (in tokens) and the factor of 2 accounts for the reduce-scatter and all-gather phases.
With two all-reduces per transformer layer and $L$ layers, the total communication per forward pass is $2L$ all-reduce operations.

For a 70B model with $d_{\text{model}} = 8192$ and TP=8 on H100 NVLink (900 GB/s), each all-reduce for a single-token decode step moves roughly 2 × 8192 × 2 bytes ≈ 32 KB.
...
```{=html}
<!-- Figure: Pipeline parallelism with 4 stages
ASCII art sketch:
...
-->
```
![Pipeline parallel layers](images/under_construction.jpg){#fig-pipeline-parallelism .lightbox}

The activation tensor passed between stages has shape $[b, d_{\text{model}}]$ (for $b$ tokens), which is much smaller than the all-reduce traffic in TP.
...
With $P$ pipeline stages, the bubble fraction is roughly $(P-1)/P$.
...
![Expert parallel MLP calculation](images/under_construction.jpg){#fig-ep-mlp .lightbox}
...
If you're processing a 128K-token prompt, the attention computation is $O(S^2)$ and the KV cache for that single request may not fit on one GPU.
